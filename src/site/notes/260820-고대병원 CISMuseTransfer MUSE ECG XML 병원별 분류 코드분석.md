...
- 김도연 추가 결론( AI 통해 코드 및 캡처 이미지로 분석 한 내용에 의견 추가!!!)
	- 기존 IIS_Receiver 의 역할을 하는 CIS Applicaiton으로 보면 된다.
	- 단지 http 프로토콜로 받은 XML을 저장하는 IIS_RECEIVER가 아닌 장비에서 네트워크 폴더로 생성한 파일에 내용을 보고 해당 site 에 대한 것으로 병원 별 네트워크 폴더에 XML 파일을 옮기도록 한다.
	- 그리고 CIS Receiver에서는 해당 네트워크 폴더를 Polling 이후 작업을 진행하는것으로 이해 하고 있음
...
## 3. 운영 설정 (캡처 화면 기준)
![Pasted image 20260820170529.png](/img/user/Pasted%20image%2020260820170529.png)
